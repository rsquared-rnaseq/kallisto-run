...
print("Processing trial {}".format(snakemake.wildcards["trial"]))
# Output path for the metadata file
metadata_path = snakemake.output[0] # "/data/Robinson-SB/metadata-trials16and14.tsv"
...
resp_data = pd.read_csv(snakemake.input[0], sep="\t")

df = pd.DataFrame(columns=columns)

cur_idx = 0

# loop through all input files, which are the abundance files
for ab in snakemake.input[1:]:
    print(ab)
